...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:Jones22*@localhost/ecom_api'
...
class OrderSchema(ma.SQLAlchemyAutoSchema):
    class Meta:
        ...
        order_date = fields.Date(format="%Y-%m-%d", required=True)
...
# Retrieve all customers with a GET method
...
# Retrieve a customer by ID with a Get method and dynamic route
...
# Update Customer by ID with a Put method
...
def update_customer(id):
    ...
    try:
        updated_data = customer_schema.load(request.json, partial=True)
    except ValidationError as e:
        return jsonify(e.messages), 400
    ...
    db.session.commit()
    return jsonify({"message": "Customer updated", "customer": customer_schema.dump(customer)})

# Delete Customer by ID with a Delete method
...
# Retrieve all Products with a Get method
...
# Retrieve a product by ID with a Get method
...
# Update Product by ID with a Put method
...
# Delete Product by ID with a Delete method
...
# Create an Order with a Post method
...
# Adding a product to an order (preventing duplicates) with a Put method
...
# Remove a Product from an ORDER with a Delete method
...
def remove_product_from_order(order_id, product_id):
    ...
    if not order or not product:
        return jsonify({"error": "Invalid order or product ID"}), 404
...
# Retrieve all Orders for a Customer with a Get method
...
# Retrieve all Products for an Order using a Get method
